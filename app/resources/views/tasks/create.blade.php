                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('tasks.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" required maxlength="255" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            @error('name')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <select name="status" id="status" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="to-do" {{ old('status', 'to-do') == 'to-do' ? 'selected' : '' }}>{{ __('To Do') }}</option>
                        ...
                        <div class="mb-4">
                            <div class="flex items-center">
                                <input type="checkbox" name="sync_with_google_calendar" id="sync_with_google_calendar" {{ old('sync_with_google_calendar') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <label for="sync_with_google_calendar" class="ml-2 block text-sm text-gray-700">{{ __('Add to Google Calendar') }}</label>
                            </div>
                            <p class="mt-1 text-xs text-gray-500">{{ __('This will create an event in your Google Calendar for this task.') }}</p>
